...
const ConferenceOverview: React.FC = () => {
  ...
  const superchairNames = activeConference?.superchairs
    ?.map(id => {
      ...
      if (superchairUsers[id]) return `${superchairUsers[id].name} ${superchairUsers[id].surname || ""}`;
      return id;
    })
    .join(", ");
  ...
  const filteredParticipants = activeConference?.pcMembers?.filter(memberId => {
    ...
    return fullName.includes(searchTerm.toLowerCase()) || memberId.includes(searchTerm);
  });
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <AppTitle text={`${activeConference?.name || 'Conference'}: Overview`} />
      </div>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ marginBottom: '20px' }}>
            <h3 style={{ marginBottom: '8px' }}>Superchair(s): {superchairNames || 'None assigned'}</h3>
            <p style={{ marginBottom: '16px' }}>Description: { activeConference?.submissionInstructions?.value || 'No description available.'}</p>
            <div style={{ 
              display: 'flex', 
              justifyContent: 'space-between', 
            ...
            <div style={{ 
              ...
              <span>Participants: {activeConference?.pcMembers?.length || 0}</span>
              <span 
          ...
          <div style={{ marginTop: '30px' }}>
            ...
            {filteredParticipants && filteredParticipants.length > 0 ? (
                  ...
                  <div key={memberId} style={{
                    ...
                    <span>
                      {isLoading 
                        ? 'Loading...' 
                        : user 
                          ? `${user.name} ${user.surname || ''}, ${getUserRoles(memberId)}` 
              ...
              <div style={{
                ...
                {searchTerm ? 'No matching participants found' : 'No participants added yet'}
              </div>
            )}
          </div>
        </div>
        ...
        <div style={{
          ...
          <div style={{ marginBottom: '20px' }}>
            <div style={{ 
              ...
              <span>Submissions: {activeConference?.tracks?.reduce((acc, track) => acc + (track.papers?.length || 0), 0) || 0}</span>
              <span 
          ...
          {(activeConference?.tracks || []).length > 0 ? (
            activeConference?.tracks.map((track, index) => (
              <div 
                ...
                onClick={() => navigate(`/conference/track/${track._id}`)}
              >
                <div style={{
                  display: 'flex',
                  justifyContent: 'space-between',
                  ...
                  <h4>
                    ...
                    {track.track_chairs && track.track_chairs.length > 0 
                      ? track.track_chairs.map(id => {
                          if (pcMemberUsers[id]) 
                            return ` ${pcMemberUsers[id].name} ${pcMemberUsers[id].surname || ""}`;
                          return " Unknown";
                        }).join(", ")
                      : " None assigned"}
                  </h4>
